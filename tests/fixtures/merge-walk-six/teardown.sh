...
# t169app is the name Adrian gives the merged stack on his own walks (top level or in DEV-TESTING); T169-MERGED/
# t169-all is the scripted walk's.
for d in "$DEST"/t169-edge "$DEST"/t169-site "$DEST"/t169-api "$DEST"/t169-store "$DEST"/t169-bus "$DEST"/t169-tools "$ROOT"/stacks/T169-MERGED/t169-all "$ROOT"/stacks/t169app "$DEST"/t169app; do
  [ -d "$d" ] || continue
...
rm -rf "$DEST"/t169-* "$DEST"/t169app "$ROOT"/stacks/T169-MERGED "$ROOT"/stacks/t169app /mnt/user/appdata/staxx-testing/t169-store
