...
foreach ($good as $g) ok('accepts '.var_export($g, true), stackman_valid_filename($g));
foreach ($bad  as $b) ok('rejects '.var_export($b, true), !stackman_valid_filename($b));
...
ok('refuses a compose filename',
   !stackman_write_file($rel, 'compose.yml', 'x', true, $err), $err);
ok('refuses a traversing name',
   !stackman_write_file($rel, '../escaped', 'x', true, $err), $err);
ok('nothing escaped', !file_exists($root.'/escaped'));

ok('refuses over the cap',
   !stackman_write_file($rel, 'big.bin', str_repeat('x', STACKMAN_FILE_MAX + 1), false, $err), $err);
ok('accepts exactly the cap',
   stackman_write_file($rel, 'big.bin', str_repeat('x', STACKMAN_FILE_MAX), false, $err), $err);
ok('leaves no temp file after a refusal', !file_exists($dir.'/big.bin.stackman-tmp'));
...
file_put_contents($dir.'/huge.bin', str_repeat('x', STACKMAN_FILE_MAX + 1));
ok('refuses to read over the cap', stackman_read_file($rel, 'huge.bin', $err) === null, $err);
ok('and says how big it is', strpos($err, 'KiB') !== false, $err);
ok('refuses a missing file', stackman_read_file($rel, 'nothere.txt', $err) === null, $err);
...
ok('renames', stackman_rename_file($rel, 'cert.der', 'cert2.der', $err), $err);
ok('the old name is gone', !file_exists($dir.'/cert.der'));
ok('the new name is there', file_exists($dir.'/cert2.der'));
ok('refuses to overwrite', !stackman_rename_file($rel, 'cert2.der', '.env', $err), $err);
ok('refuses to rename onto a compose file',
   !stackman_rename_file($rel, 'cert2.der', 'compose.yml', $err), $err);

/* ------------------------------------------------------------ deleting -- */

ok('deletes', stackman_delete_file($rel, 'big.bin', $err), $err);
ok('the file is gone', !file_exists($dir.'/big.bin'));
ok('refuses a missing file', !stackman_delete_file($rel, 'big.bin', $err), $err);

mkdir($dir.'/sub', 0755);
file_put_contents($dir.'/sub/one.txt', 'x');
ok('refuses to delete a folder', !stackman_delete_file($rel, 'sub', $err), $err);
...
ok('extras refuses a missing stack', stackman_stack_extras('nosuchstack', $err) === null, $err);

/* -------------------------------------------------------------- delete -- */

ok('unconfirmed delete refuses', !stackman_delete_stack($rel, $err, false), $err);
...
ok('confirmed delete removes the lot', stackman_delete_stack($rel, $err, true), $err);
