...
x-unraid:
  imported:
    from: unraid-template   # community-applications | unraid-template | docker-image | running-container
    on: 2026-08-30
    id: my-jellyfin.xml     # optional — which template, catalogue entry or image this came from
    name: Jellyfin          # optional — its display name at import time
```

Stamped once, the moment a file is generated by converting a Community Applications entry, a user's
own Unraid template, or an image reference — never written by hand and never touched again
afterwards. It records how the file came to exist, not something to change. A stack that was
hand-written from scratch carries no `imported` block at all.

`id` and `name` are optional and let the Import list recognise a stack that has since been renamed:
`id` is the source's own identifier (a template's file name, a catalogue entry's name or URL, or the
image reference an image import used), and `name` is the display name it carried at the time.
...
the scheme, the path, `overview`, `display` — still cannot be, because nothing yet renders the
whole of `x-unraid` as form fields. Changing those still means opening the Compose view.
