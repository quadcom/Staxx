...
| Notify me | One is found / One is installed / One fails, each on or off | One fails is on; the other two are off | Three separate switches for when StaXX sends you an Unraid notification: when a check finds something waiting, when an update has installed, and when one fails. Any mix of the three can be on at once. A container can take itself out of these messages in its own settings — see [choosing how a container updates](update-policy.md). |
