...
|---|---|---|
| Green, e.g. "Up 5 minutes" | Running. | Nothing. |
...
| Grey, "stopped" | Not running. | Start it when you want it. |
...
| Red, "Dead" | Broken. | Worth a look. Try restarting it. |
| "unknown", plain grey text | StaXX cannot ask the container service right now. | Wait for it to answer again. |
| Dashed, "Starting…" or "Downloading image…" | A command you asked for is still running, or the image is downloading on a first start. | Wait. Click the pill to watch the log as it happens. |

Hover a pill to see whether it means the container is going or the app inside says it is working.
See [health checks](editing-a-stack.md) for how to add the second kind.

## Update state

![A running pill beside an orange update ready pill, the update pill outlined](../images/guide/marks-update-pill.png)

Sits beside the running pill, empty when nothing newer was found. See
[update checking](updates.md) for the cadence, the countdown, and the author's own published
example.

| Wording | Meaning |
|---|---|
| *(nothing)* | Nothing newer found, or nothing checked yet. Press **Check this image again** in the row menu to settle which. |
...
Only `update ready` and its version forms are a button to press. `update ready` and `rebuild ready`
are shown in a stronger colour.

![A stack row's own menu open, with Check this image again outlined](../images/guide/marks-row-menu.png)

### The hover card

Hover a pill, or press it on a touch screen, for a small card: the version running now, the version
on offer, when it was last checked, and when it is next due.

## Marks under the name

| Example | Mark | Meaning |
|---|---|---|
| <img src="../images/guide/marks-needs-review.png" alt="A drawing pin icon under a stack name"> | Drawing pin | One or more services is fixed to one exact build. Hover it to see which. |
| Green circular arrows | Updates itself | This container will install an update on its own, whether from its own setting or the server's. See [choosing how a container updates](update-policy.md). |
...
| Orange triangle | Ports do nothing | A service is on a network that gives it its own address. Hover it to see which service. |
...
a graphics card. It stays even while the stack is stopped.
...
| Orange triangle | Beside the image name, in the Services column | The file now names a different image than the one running. |
...
| Example | Tag | Meaning |
|---|---|---|
| ![Three stack rows, each with an orange "needs review" tag sitting next to the stack's name](../images/guide/marks-needs-review.png) | `needs review` | [Imported](bringing-in-a-container.md) and not checked over yet. Confirm it from the row's own menu before it will start or update itself. |
| Orange outlined button | `waiting to confirm` | Shown right after a handover, when an older container has switched off and this one is running in its place. Check the app still works, then answer in the row's own menu. |
| Small lightning bolt | *(no text; hover for detail)* | Starts at boot. Hover it to see whether that is the whole stack, only part of it, or nothing, and whether a delay is set before the next one starts. |

## Broken stack marks

The strongest mark on the list — it replaces the app's own logo rather than sitting beside it. It
means Compose could not read that stack's file, or found no file at all. The row states which, in
...
Open the editor from the triangle and fix the file. The row returns to normal on the next save. The
same triangle appears on a [folder row](folders.md) when a stack inside it is broken.
...
|---|---|
| The image already checks itself | Nothing offered. |
...
StaXX tries the candidate check once against the running container before offering it. If the
command cannot run at all, StaXX offers nothing.
