...
All in `include/Defines.php`, modelled on `stackman_hub_search()` (`:340-379`) and holding its
contract: **an empty result for every failure**, no exception, no logging.

- `stackman_hub_json(string $url, array $headers = []): array` — the three lines currently duplicated
  at `:307-310` and `:358-361` (build URL → `stackman_sh('curl -fsSL --max-time 8 …', 12)` →
...
- `stackman_hub_repo(string $repo): array` — one request to
...
- `stackman_registry_config(string $repo): array` — the three-request chain, and **only ever the
...
- `stackman_local_image_config(string $ref): array` — the same three fields from
  `docker image inspect --format '{{json .Config}}'`, no network at all. First use of `--format json`
  outside `stackman_compose_state()`; the only other `docker inspect` here is template-formatted
  (`Stacks.php:1213`).
- `stackman_server_timezone(): string` — `readlink -f /etc/localtime` with the zoneinfo prefix
...
`stackman_image_facts()` is the small orchestrator: **off** when `IMAGE_LOOKUP` is `false` (returned
...
reason written at the top of that file — and a `<select>` in `stack.manager.settings.page` copying
...
- `call('image-facts', {image, source}, 30000)` — 30 s because the fallback route is three chained
  ...
  in about a second.
- While it runs, a short line in the footer (`#stackman-ca-msg`) saying the documentation is being
...
`stackman_hub_repo('linuxserver/jellyfin')`, `stackman_registry_config('library/nginx')`,
`stackman_local_image_config()` on something present, and each of them on `'../../etc'` and `''` —
...
node --check src/stack.manager/usr/local/emhttp/plugins/stack.manager/javascript/image-import.js
...
| `default.cfg`, `stack.manager.settings.page` | `IMAGE_LOOKUP` and its prose |
