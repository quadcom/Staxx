<?PHP
/* StaXX — builds the Community Applications catalogue cache.
 * Copyright 2026, StaXX contributors.
 *
 * Run detached by staxx_ca_refresh_start() (see include/CA.php), never by a
 * page render: it downloads a 24 MB feed and can take the best part of a
 * minute. Can also be run by hand on the server —
 *   php /usr/local/emhttp/plugins/staxx/scripts/ca-index.php
...
function staxx_ca_index_status(string $state, string $message, int $built = 0, int $count = 0): void {
  ...
  staxx_ca_index_write_json(STAXX_CA_STATUS, $data);
}

/** Recursive delete, for clearing out a stale ca.new or ca.old directory. */
function staxx_ca_index_rmtree(string $dir): void {
  if (!is_dir($dir) || is_link($dir)) { @unlink($dir); return; }
  foreach (scandir($dir) ?: [] as $item) {
    if ($item === '.' || $item === '..') continue;
    $path = $dir.'/'.$item;
    is_dir($path) && !is_link($path) ? staxx_ca_index_rmtree($path) : @unlink($path);
  }
  @rmdir($dir);
}
 ...
 */
function staxx_ca_index_download(string $url, string $dest): bool {
  $fh = @fopen($dest, 'wb');
 ...
 */
function staxx_ca_index_feed_stamp(): int {
  foreach ([
 ...
 * known sections (STAXX_CA_INDEX_SECTIONS) off the front: two known
 ...
 * but is not.
 */
function staxx_ca_index_category(string $raw): array {
  $raw = trim($raw);
  if ($raw === '') return [];
  ...
  foreach (explode(';', $raw) as $part) {
    ...
    while ($i < $n) {
      ...
      if (!in_array($token, STAXX_CA_INDEX_SECTIONS, true)) {
      ...
      $next = $tokens[$i + 1];
      if (in_array($next, STAXX_CA_INDEX_SECTIONS, true)) {
      ...
      $j = $i + 1;
      while ($j < $n && !in_array($tokens[$j], STAXX_CA_INDEX_SECTIONS, true)) $j++;
 ...
 */
function staxx_ca_truthy($v): bool {
 ...
 */
function staxx_ca_index_entry(array $app): ?array {
  ...
  if (staxx_ca_truthy($app['Blacklist'] ?? null) || staxx_ca_truthy($app['CABlacklist'] ?? null)) return null;
  if (staxx_ca_truthy($app['hideFromCA'] ?? null) || staxx_ca_truthy($app['HideFromCA'] ?? null) || staxx_ca_truthy($app['hideFromWeb'] ?? null)) return null;
  ...
  foreach ($catList as $catRaw) {
    foreach (staxx_ca_index_category((string)$catRaw) as $cat) {
  ...
  if (staxx_ca_truthy($app['Deprecated'] ?? null)) $entry['dep'] = 1;
 ...
 */
function staxx_ca_index_split(string $feedPath, string $outDir, int $feedStamp = 0): array {
    ...
    for ($p = 0; $p < $len; $p++) {
      ...
      if ($c === '}') {
        ...
        if ($depth === 0) {
          ...
          $built   = is_array($decoded) ? staxx_ca_index_entry($decoded) : null;
          if ($built !== null) {
  ...
  staxx_ca_index_write_json($outDir.'/index.json', [
    'built'      => time(),
    ...
    // download when it has not moved — see staxx_ca_index_feed_stamp().
    'feed_ts'    => $feedStamp,
...
if (!is_dir(STAXX_CA_LOCK)) @mkdir(STAXX_CA_LOCK, 0755, true);

echo "StaXX: rebuilding the Community Applications catalogue.\n";
staxx_ca_index_status('building', 'Downloading the applications catalogue.');
...
try {
  ...
  $feedStamp = staxx_ca_index_feed_stamp();
  $haveStamp = 0;
  $haveVer   = 0;
  if (is_file(STAXX_CA_INDEX)) {
    $existing  = json_decode((string)@file_get_contents(STAXX_CA_INDEX), true);
  ...
  if ($feedStamp > 0 && $feedStamp === $haveStamp && $haveVer === STAXX_CA_INDEX_VERSION) {
    ...
    @touch(STAXX_CA_INDEX);
    $existingCount = is_array($existing ?? null) ? (int)($existing['count'] ?? 0) : 0;
    staxx_ca_index_status('ready', 'The applications catalogue is up to date.', time(), $existingCount);
    echo "Done.\n";
  ...
  $feedTmp = STAXX_CA_DIR.'/feed.json.download';
  $sources = [
  ...
  foreach ($sources as $url) {
    echo "Fetching $url ...\n";
    if (staxx_ca_index_download($url, $feedTmp)) { $downloaded = true; break; }
  ...
  $newDir = STAXX_CA_DIR.'.new';
  staxx_ca_index_rmtree($newDir);
  mkdir($newDir, 0755, true);

  $result = staxx_ca_index_split($feedTmp, $newDir, $feedStamp);
  @unlink($feedTmp);
