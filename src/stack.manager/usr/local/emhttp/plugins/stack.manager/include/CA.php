<?PHP
/* Stack Manager — Community Applications catalogue.
 ...
 *                history, moderator notes) stripped, for stackman_ca_app()
...
?>
<?
if (defined('STACKMAN_CA_DIR')) return;
...
const STACKMAN_CA_DIR    = '/tmp/stack.manager/ca';
const STACKMAN_CA_APPS   = STACKMAN_CA_DIR.'/apps.jsonl';
const STACKMAN_CA_INDEX  = STACKMAN_CA_DIR.'/index.json';
const STACKMAN_CA_STATUS = STACKMAN_CA_DIR.'/status.json';

/** Atomic-mkdir lock, the same trick scripts/stats-collector.sh uses. */
const STACKMAN_CA_LOCK = STACKMAN_CA_DIR.'/.lock';
 ...
 */
function stackman_ca_index_data(): array {
  static $data = null;
  if ($data !== null) return $data;

  $empty = ['built' => 0, 'count' => 0, 'categories' => [], 'apps' => []];

  $raw = @file_get_contents(STACKMAN_CA_INDEX);
 ...
 */
function stackman_ca_status(): array {
  $raw    = @file_get_contents(STACKMAN_CA_STATUS);
  ...
  $usable = count(stackman_ca_index_data()['apps']) > 0;

  $count = (int)($status['count'] ?? 0);
  if (!$count) $count = (int)(stackman_ca_index_data()['count'] ?? 0);
  ...
  if ($stale) {
    ...
    $statusTime = @filemtime(STACKMAN_CA_STATUS);
 ...
 */
function stackman_ca_refresh_start(): void {
  if (is_dir(STACKMAN_CA_LOCK)) {
    ...
    if (time() - (int)@filemtime(STACKMAN_CA_LOCK) < 1800) return;
  ...
  if (!@mkdir(STACKMAN_CA_LOCK, 0755, true)) return;

  $log = '/tmp/stack.manager/ca-build.log';
  $cmd = 'php '.escapeshellarg(STACKMAN_ROOT.'/scripts/ca-index.php');

  // setsid detaches the build from this request exactly as stackman_start_job()
...
/**
 * One app's full JSON entry, by the ordinal stackman_ca_search() gave out.
 *
 ...
 */
function stackman_ca_app(int $i): ?array {
  $entry = stackman_ca_index_data()['apps'][$i] ?? null;
 ...
 *  matches nothing. */
function stackman_ca_categories(): array {
  $categories = stackman_ca_index_data()['categories'];
