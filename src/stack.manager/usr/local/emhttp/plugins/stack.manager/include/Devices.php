<?PHP
/* Stack Manager — what hardware this server can hand to a container.
...
if (defined('STACKMAN_SERIAL_DIR')) return;
...
// does not.
define('STACKMAN_SERIAL_DIR', '/dev/serial/by-id');

/* ------------------------------------------------------- naming what we find -- */

/** Vendor id from stackman_gpu_nodes() to something a person would say. */
const STACKMAN_GPU_LABELS = [
 ...
 */
function stackman_device_pretty(string $file): string {
 ...
 */
function stackman_device_catalogue(): array {
  static $cached = null;
  ...
  if ($render) {
    $named = [];
    foreach (array_keys($vendors) as $v) $named[] = STACKMAN_GPU_LABELS[$v] ?? $v;
    ...
    foreach ($render as $node => $vendor) {
      $gfx[] = [
        ...
        'label'      => (STACKMAN_GPU_LABELS[$vendor] ?? $vendor).' only ('.$node.')',
  ...
  $serial = [];
  $seen   = [];
  foreach ((array)@glob(STACKMAN_SERIAL_DIR.'/*') as $link) {
    ...
    $serial[] = [
      ...
      'container' => $short,
      'label'     => stackman_device_pretty(basename($link)),
 ...
 */
function stackman_device_host(string $item): string {
  $item = trim($item);
 ...
 *
 * Its own indentation walk rather than stackman_yaml_flatten(), which discards
 * every sequence it meets — that is where device entries live, so it cannot
 * answer this. Widening it instead would change what stackman_compose_meta()
 ...
 */
function stackman_compose_devices(string $yaml): array {
  $out     = [];
  ...
  foreach (explode("\n", $yaml) as $raw) {
    ...
    if ($isItem) {
      if ($devAt >= 0 && $indent > $devAt) {
        $host = stackman_device_host(substr($body, 1));
 ...
 */
function stackman_device_claims(): array {
  $out = [];

  foreach (stackman_scan_stacks()['stacks'] as $s) {
    $file = stackman_find_compose_file($s['dir']);
    if ($file === '') continue;

    $yaml = (string)@file_get_contents($file);
    if ($yaml === '') continue;

    foreach (stackman_compose_devices($yaml) as $service => $paths) {
 ...
 * @return string[]
 */
function stackman_device_paths(): array {
  $out = [];
  ...
  // missing hardware.
  foreach (stackman_device_catalogue() as $group) {
...
function stackman_devices(): array {
  return [
    'groups'  => stackman_device_catalogue(),
    'claims'  => stackman_device_claims(),
    'present' => stackman_device_paths(),
  ];
}
?>
