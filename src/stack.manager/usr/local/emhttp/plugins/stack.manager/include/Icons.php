<?PHP
/* Stack Manager — icons for stacks and containers.
...
const STACKMAN_ICON_CDN = 'https://cdn.jsdelivr.net/gh/selfhst/icons';
 ...
 */
const STACKMAN_ICON_STORE = STACKMAN_CFG_DIR.'/icons';
 ...
 */
const STACKMAN_ICON_SERVE = '/var/local/emhttp/plugins/'.STACKMAN_PLUGIN.'/icons';
const STACKMAN_ICON_BASE  = '/state/plugins/'.STACKMAN_PLUGIN.'/icons';
...
const STACKMAN_ICON_INDEX = STACKMAN_ICON_STORE.'/_index.json';

/** How stale the index may get before it is fetched again. */
const STACKMAN_ICON_INDEX_TTL = 7 * 86400;

/** File extensions that may be written into the cache. */
const STACKMAN_ICON_EXTS = ['svg', 'png', 'webp', 'jpg', 'jpeg', 'gif', 'ico'];

/* ------------------------------------------------------------- settings -- */

/** Whether the plugin is allowed to fetch icons from the internet. */
function stackman_icon_fetching(): bool {
  return (stackman_cfg()['ICON_FETCH'] ?? 'true') !== 'false';
}
 ...
 */
function stackman_icon_norm(string $s): string {
 ...
 */
function stackman_icon_safe_ref(string $ref): bool {
 ...
 */
function stackman_icon_index(): array {
  static $index = null;
  if ($index !== null) return $index;

  $index = ['refs' => [], 'alias' => [], 'order' => []];

  $raw = @file_get_contents(STACKMAN_ICON_INDEX);
...
function stackman_icon_index_stale(): bool {
  $when = @filemtime(STACKMAN_ICON_INDEX);
  return $when === false || (time() - $when) > STACKMAN_ICON_INDEX_TTL;
}
 ...
 */
function stackman_icon_index_refresh(): bool {
  if (!stackman_icon_fetching()) return false;

  $raw = stackman_icon_get(STACKMAN_ICON_CDN.'/index.json', 20);
  ...
  foreach ($list as $entry) {
    ...
    if (!stackman_icon_safe_ref($ref)) continue;
    ...
    $name = stackman_icon_norm((string)($entry['Name'] ?? ''));
  ...
  return stackman_icon_write(STACKMAN_ICON_INDEX,
 ...
 */
const STACKMAN_ICON_LEAD = '/^(arch|container|docker|unraid)-/';
const STACKMAN_ICON_TAIL = '/-(docker|server|app|aio|ce|oss|community|amd64|arm64)$/';
 ...
 */
function stackman_icon_prefixed(array $order, string $prefix): array {
  $lo = 0;
  $hi = count($order);
  while ($lo < $hi) {
    $mid = intdiv($lo + $hi, 2);
 ...
 */
function stackman_icon_lookup(string $candidate): string {
  $index = stackman_icon_index();
  if (!$index['refs']) return '';

  $c = stackman_icon_norm($candidate);
  if ($c === '') return '';
  ...
  $hits = stackman_icon_prefixed($index['order'], $c.'-');
 ...
 * @return string[]
 */
function stackman_icon_candidates(string $image): array {
  ...
  $push = function (string $s) use (&$out) {
    $s = stackman_icon_norm($s);
    if ($s === '') return;
    $out[] = $s;
    if (preg_match(STACKMAN_ICON_LEAD, $s)) $out[] = (string)preg_replace(STACKMAN_ICON_LEAD, '', $s);
    if (preg_match(STACKMAN_ICON_TAIL, $s)) $out[] = (string)preg_replace(STACKMAN_ICON_TAIL, '', $s);
    $out[] = str_replace('-', '', $s);
  };
 ...
 */
function stackman_icon_match(string $image, string $service = '', string $stack = ''): string {
  $tries = stackman_icon_candidates($image);
  foreach ([$service, $stack] as $extra) {
    if ($extra !== '') $tries[] = stackman_icon_norm($extra);
  }

  foreach (array_unique($tries) as $candidate) {
    $ref = stackman_icon_lookup($candidate);
    if ($ref !== '') return $ref;
  }
  return '';
}
...
function stackman_icon_write(string $path, string $body): bool {
  $dir = dirname($path);
...
function stackman_icon_get(string $url, int $seconds = 10): ?string {
 ...
 */
function stackman_icon_url(string $ref): string {
  if (!stackman_icon_safe_ref($ref)) return '';

  foreach (STACKMAN_ICON_EXTS as $ext) {
    $served = STACKMAN_ICON_SERVE.'/'.$ref.'.'.$ext;
    if (is_file($served)) return STACKMAN_ICON_BASE.'/'.$ref.'.'.$ext;

    $kept = STACKMAN_ICON_STORE.'/'.$ref.'.'.$ext;
    if (is_file($kept)) {
      if (!is_dir(STACKMAN_ICON_SERVE)) @mkdir(STACKMAN_ICON_SERVE, 0755, true);
      if (@copy($kept, $served)) return STACKMAN_ICON_BASE.'/'.$ref.'.'.$ext;
    }
  }
  return '';
}
...
function stackman_icon_store(string $ref, string $ext, string $body): bool {
  if (!stackman_icon_safe_ref($ref)) return false;
  if (!in_array($ext, STACKMAN_ICON_EXTS, true)) return false;

  if (!stackman_icon_write(STACKMAN_ICON_STORE.'/'.$ref.'.'.$ext, $body)) return false;
  stackman_icon_write(STACKMAN_ICON_SERVE.'/'.$ref.'.'.$ext, $body);
  return true;
}
 ...
 */
function stackman_icon_fetch(string $ref, string $remote = ''): string {
  if (!stackman_icon_fetching()) return '';
  if (!stackman_icon_safe_ref($ref)) return '';

  $cached = stackman_icon_url($ref);
  if ($cached !== '') return $cached;

  $sources = [];
  if ($remote !== '') {
    $ext = strtolower((string)pathinfo((string)parse_url($remote, PHP_URL_PATH), PATHINFO_EXTENSION));
    if (!in_array($ext, STACKMAN_ICON_EXTS, true)) $ext = 'png';
    $sources[] = [$remote, $ext];
  } else {
    $have = stackman_icon_index()['refs'][$ref] ?? '';
    if ($have === '') return '';
    if (strpos($have, 's') !== false) $sources[] = [STACKMAN_ICON_CDN.'/svg/'.$ref.'.svg', 'svg'];
    if (strpos($have, 'p') !== false) $sources[] = [STACKMAN_ICON_CDN.'/png/'.$ref.'.png', 'png'];
  }

  foreach ($sources as [$url, $ext]) {
    $body = stackman_icon_get($url);
    if ($body === null) continue;
    if (stackman_icon_store($ref, $ext, $body)) return STACKMAN_ICON_BASE.'/'.$ref.'.'.$ext;
  }
  return '';
}
 ...
 */
function stackman_icon_sweep(array $wanted, int $budget = 10): array {
  if (!stackman_icon_fetching()) return ['icons' => [], 'done' => true];
  ...
  if (stackman_icon_index_stale()) stackman_icon_index_refresh();
  ...
  foreach ($wanted as $item) {
    ...
    $url = stackman_icon_fetch($ref, (string)($item['remote'] ?? ''));
 ...
 *           knows, and stackman_icon_fetch() works it out from $ref
 */
function stackman_icon_resolve(string $icon, string $dir = '', string $image = '',
  ...
  if ($icon !== '') {
    ...
    if (preg_match('#^https?://#i', $icon)) {
      $ref = 'url-'.md5($icon);
      return ['fa' => '', 'ref' => $ref, 'url' => stackman_icon_url($ref), 'remote' => $icon];
    }
    ...
    if ($dir !== '' && strpos($icon, '..') === false) {
      ...
      if (in_array($ext, STACKMAN_ICON_EXTS, true) && is_file($path)) {
        $ref = 'local-'.md5($path.'|'.(string)@filemtime($path));
        $url = stackman_icon_url($ref);
        if ($url === '') {
          $body = @file_get_contents($path);
          if ($body !== false && stackman_icon_store($ref, $ext, $body)) {
            $url = STACKMAN_ICON_BASE.'/'.$ref.'.'.$ext;
          }
        }
    ...
    $ref = stackman_icon_norm($icon);
    if (stackman_icon_safe_ref($ref) && isset(stackman_icon_index()['refs'][$ref])) {
      return ['fa' => '', 'ref' => $ref, 'url' => stackman_icon_url($ref), 'remote' => ''];
    }
    return $none;
  }

  $ref = stackman_icon_match($image, $service, $stack);
  if ($ref === '') return $none;
 ...
 * .stackman-tile--N rules in the stylesheet.
 */
const STACKMAN_ICON_COLOURS = 10;
 ...
 */
function stackman_icon_initials(string $name): array {
  ...
  // same colour.
  return ['text' => $text, 'colour' => (int)(crc32(strtolower($name)) % STACKMAN_ICON_COLOURS)];
}
?>
