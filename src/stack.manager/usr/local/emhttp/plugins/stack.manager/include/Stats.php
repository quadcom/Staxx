<?PHP
/* Stack Manager — live statistics for stacks and their containers.
...
if (defined('STACKMAN_STATS_DIR')) return;

define('STACKMAN_STATS_DIR', '/tmp/stack.manager/stats');
define('STACKMAN_STATS_SCRIPT', STACKMAN_ROOT.'/scripts/stats-collector.sh');
 ...
 */
function stackman_stats_touch(): void {
  if (!is_dir(STACKMAN_STATS_DIR)) @mkdir(STACKMAN_STATS_DIR, 0755, true);
  @file_put_contents(STACKMAN_STATS_DIR.'/watch', (string)time());

  if (stackman_stats_collector_running()) return;
  if (!is_file(STACKMAN_STATS_SCRIPT)) return;

  // Detached, with its input and output disconnected, so it outlives this
  // request instead of being killed when PHP finishes.
  @exec('setsid sh '.escapeshellarg(STACKMAN_STATS_SCRIPT).' '
        .escapeshellarg(STACKMAN_STATS_DIR).' </dev/null >/dev/null 2>&1 &');
}

function stackman_stats_collector_running(): bool {
  $pidFile = STACKMAN_STATS_DIR.'/collector.pid';
 ...
 */
function stackman_bytes(string $text): float {
  $text = trim($text);
...
function stackman_byte_pair(string $text): array {
  $parts = explode('/', $text);
  return [
    stackman_bytes($parts[0] ?? ''),
    stackman_bytes($parts[1] ?? ''),
  ];
}

function stackman_percent(string $text): float {
 ...
 */
function stackman_stats_index(): array {
  $byId = [];
  $byName = [];

  foreach (@file(STACKMAN_STATS_DIR.'/ps.raw', FILE_IGNORE_NEW_LINES) ?: [] as $line) {
    $cols = explode("\t", $line);
 ...
 */
function stackman_stats_containers(): array {
  $out = [];

  foreach (@file(STACKMAN_STATS_DIR.'/docker.raw', FILE_IGNORE_NEW_LINES) ?: [] as $line) {
    $row = json_decode($line, true);
    if (!is_array($row) || !isset($row['Name'])) continue;

    [$memUsed, $memLimit] = stackman_byte_pair((string)($row['MemUsage'] ?? ''));
    [$netRx,   $netTx]    = stackman_byte_pair((string)($row['NetIO']    ?? ''));
    [$blkRead, $blkWrite] = stackman_byte_pair((string)($row['BlockIO']  ?? ''));

    $out[$row['Name']] = [
      'id'       => (string)($row['Container'] ?? ''),
      'name'     => (string)$row['Name'],
      'cpu'      => stackman_percent((string)($row['CPUPerc'] ?? '')),
      'memUsed'  => $memUsed,
      'memLimit' => $memLimit,
      'memPct'   => stackman_percent((string)($row['MemPerc'] ?? '')),
 ...
 */
function stackman_stats_intel(): ?array {
  $raw = (string)@file_get_contents(STACKMAN_STATS_DIR.'/intel.raw');
  if ($raw === '') return null;
 ...
 */
function stackman_stats_intel_gpu(): array {
  $empty = ['byContainer' => [], 'engines' => [], 'total' => 0.0, 'present' => false];

  $sample = stackman_stats_intel();
 ...
 */
function stackman_gpu_sample(string $file): array {
 ...
 */
function stackman_stats_gpu_procs(): array {
  $now  = stackman_gpu_sample(STACKMAN_STATS_DIR.'/gpuproc.raw');
  $then = stackman_gpu_sample(STACKMAN_STATS_DIR.'/gpuproc.prev');
 ...
 */
function stackman_gpu_nodes(): array {
  static $nodes = null;
 ...
 */
function stackman_gpu_support(): array {
  ...
  return [
    ...
    'nvidia' => [
      'ok'  => stackman_have('nvidia-smi'),
      'why' => stackman_have('nvidia-smi')
        ? '' : 'nvidia-smi is not installed (the Nvidia Driver plugin provides it)',
    ],
  ];
}

function stackman_have(string $binary): bool {
  static $seen = [];
  if (!isset($seen[$binary])) {
    $seen[$binary] = stackman_sh('command -v '.escapeshellarg($binary), 5) !== '';
  }
  return $seen[$binary];
}
 ...
 */
function stackman_stats_gpu_mapped(): array {
  $nodes = stackman_gpu_nodes();
  $out   = [];

  foreach (@file(STACKMAN_STATS_DIR.'/devices.raw', FILE_IGNORE_NEW_LINES) ?: [] as $line) {
    $cols = explode("\t", $line);
 ...
 * than renderD129 — so the node names stackman_gpu_nodes() knows about have to
 * be turned into numbers. /sys/class/drm/<node>/dev holds "226:129".
 *
 * @return array<int,string> minor => 'intel' | 'amd' | 'nvidia'
 */
function stackman_gpu_minors(): array {
  static $minors = null;
  if ($minors !== null) return $minors;

  $minors = [];
  foreach (stackman_gpu_nodes() as $node => $vendor) {
 ...
 */
function stackman_stats_gpu_clients(): array {
  $out    = ['intel' => 0, 'amd' => 0, 'nvidia' => 0];
  $minors = stackman_gpu_minors();

  foreach (@file(STACKMAN_STATS_DIR.'/gpuclients.raw', FILE_IGNORE_NEW_LINES) ?: [] as $line) {
    $f = explode(' ', trim($line));
 ...
 */
function stackman_stats_gpu_media(): array {
  $samples = [];

  foreach (@file(STACKMAN_STATS_DIR.'/gpumetrics.raw', FILE_IGNORE_NEW_LINES) ?: [] as $line) {
    $f = explode(' ', trim($line));
    ...
    $offset = STACKMAN_GPU_METRICS_MM[$header['format'].'.'.$header['content']] ?? null;
 ...
 */
function stackman_stats_amd(): ?array {
  $media = null;
  $peak  = 0.0;
  foreach (stackman_stats_gpu_media() as $minor => $figures) {
    if ((stackman_gpu_minors()[$minor] ?? '') !== 'amd') continue;
  ...
  $raw = trim((string)@file_get_contents(STACKMAN_STATS_DIR.'/amd.raw'));
 ...
 */
function stackman_stats_snapshot(): array {
  stackman_stats_touch();

  $index      = stackman_stats_index();
  $containers = stackman_stats_containers();
  $intel      = stackman_stats_intel_gpu();
  $amd        = stackman_stats_amd();
  $mapped     = stackman_stats_gpu_mapped();
  $support    = stackman_gpu_support();
  $gpuProcs   = stackman_stats_gpu_procs();
  $clients    = stackman_stats_gpu_clients();
  ...
  $sampledAt = (int)trim((string)@file_get_contents(STACKMAN_STATS_DIR.'/docker.at'));
  ...
  return [
    ...
    'now'       => time(),
    'warming'   => $sampledAt === 0,
    'collector' => stackman_stats_collector_running(),
  ];
}
?>
