<?PHP
/* Stack Manager — the stack table, rendered in exactly one place.
...
require_once '/usr/local/emhttp/plugins/stack.manager/include/Icons.php';

// NO "already loaded?" guard here, deliberately.
//
// The obvious one — `if (function_exists('stackman_render_rows')) return;` —
 ...
 */
function stackman_address_html(array $addresses): string {
  ...
  foreach ($addresses as $a) {
    ...
    $out[] = '<span class="stackman-addr"'.$title.'>'.$name.':'
           . '<span class="stackman-addr-ports">'
           . htmlspecialchars(implode(', ', $ports)).'</span></span>';
  }

  return $out ? implode('', $out) : '<span class="stackman-sub">—</span>';
}
 ...
 *
 * @param array $containers rows from stackman_container_index()
 */
function stackman_merged_addresses(array $containers): array {
  $net = stackman_container_net();
 ...
 */
function stackman_state_pill(array $s, bool $canRun): string {
  if (!$canRun) {
    return '<span class="stackman-sub">'._('unknown').'</span>';
  }
  if (!empty($s['running'])) {
    return '<span class="stackman-pill stackman-pill--up">'
         . htmlspecialchars((string)$s['status']).'</span>';
  }
  if ((string)$s['status'] !== '') {
    return '<span class="stackman-pill">'.htmlspecialchars((string)$s['status']).'</span>';
  }
  return '<span class="stackman-pill stackman-pill--down">'._('stopped').'</span>';
}

/** The "3 stacks · 1 running" line under a folder name. */
function stackman_folder_sub(int $count, int $running): string {
...
function stackman_stack_sub(int $count, int $running): string {
 ...
 */
function stackman_container_pill(array $c): string {
  if (!$c['exists']) {
    return '<span class="stackman-pill stackman-pill--down">'._('not created').'</span>';
  }
  ...
  switch ($c['state']) {
    case 'running':
      return '<span class="stackman-pill stackman-pill--up">'.$status.'</span>';
    case 'restarting':
    case 'removing':
    case 'paused':
      return '<span class="stackman-pill stackman-pill--warn">'.$status.'</span>';
    case 'dead':
      return '<span class="stackman-pill stackman-pill--bad">'.$status.'</span>';
    default:                                   // exited, created
      return '<span class="stackman-pill stackman-pill--down">'.$status.'</span>';
  }
}
 ...
 *                          exists:bool}>
 */
function stackman_stack_children(array $s): array {
  $containers = stackman_stack_containers($s);
  ...
  $declared = $s['file'] !== '' ? stackman_compose_meta($s['file'])['services'] : [];
 ...
 *
 * @param array  $icon from stackman_icon_resolve()
...
function stackman_icon_tile(array $icon, string $name): string {
  ...
  $tile = stackman_icon_initials($name);
  ...
  return '<span class="stackman-tile stackman-tile--'.$tile['colour'].'"'.$ref.'>'
 ...
 *
 * @param array $kids from stackman_stack_children()
 */
function stackman_stack_tile(array $s, array $kids): string {
  $own = (string)($s['x']['icon'] ?? '');
  if ($own !== '') {
  ...
  $tiles = [];
  foreach ($kids as $kid) {
    $tiles[] = stackman_icon_tile(
      stackman_icon_resolve($kid['icon'], $s['dir'], $kid['image'], $kid['service'], $s['name']),
  ...
  if ($extra > 0) {
    ...
    $tiles[] = '<span class="stackman-tile stackman-tile--more">+'.($extra + 1).'</span>';
  }

  return '<span class="stackman-mosaic">'.implode('', $tiles).'</span>';
}
 ...
 */
function stackman_icon_wanted(): array {
  $wanted = [];
  ...
  foreach (stackman_list_stacks() as $s) {
    ...
    $own = (string)($s['x']['icon'] ?? '');
    if ($own !== '') {
      $add(stackman_icon_resolve($own, $s['dir']));
      // A stack that names its own icon does not need its children's, but the
      // container rows underneath it still show them.
    }

    foreach (stackman_stack_children($s) as $kid) {
      $add(stackman_icon_resolve($kid['icon'], $s['dir'], $kid['image'],
                                 $kid['service'], $s['name']));
    }
  }
 ...
 * still line up the way a <table>'s did — see stack.manager.css for the track
 ...
 *
 *   .stackman-group--folder   a folder heading plus every stack filed under it
 *   .stackman-group--stack    an expandable stack's own row plus its containers
 *
 ...
 *
 * @param array $rows   from stackman_folder_layout()
 * @param bool  $canRun whether docker and compose are both usable
 */
function stackman_render_rows(array $rows, bool $canRun): string {
  ob_start();

  if (!$rows):
?>
        <div class="stackman-row stackman-empty-row" role="row">
          <span class="stackman-cell" role="gridcell">
...
<?
  ...
  // Whether a <div class="stackman-group stackman-group--folder"> is currently
  // open. stackman_folder_layout() lists every folder's members immediately
...
<?
      ...
      $inFolderGroup = true;
?>
      <div class="stackman-group stackman-group--folder" role="presentation" data-folder-group="<?= htmlspecialchars($row['id']) ?>">
        <div class="stackman-row stackman-folder-row" role="row" aria-level="1"
          ...
          <span class="stackman-cell stackman-cell--span4" role="gridcell">
            <!-- Flex lives on a div, never on the cell itself. -->
            <div class="stackman-namebox">
              <button type="button" class="stackman-chevron"
              ...
              <span class="stackman-iconwrap">
                <button type="button" class="stackman-icon"
                        data-menu="folder"
                ...
                <span class="stackman-spinner"><i class="fa fa-refresh fa-spin"></i></span>
                <span class="stackman-dot<?= $row['running'] ? ' stackman-dot--up' : '' ?>"></span>
              </span>

              <span class="stackman-nameinfo">
                <span class="stackman-folder-name"><?= htmlspecialchars($row['name']) ?></span>
                <span class="stackman-sub" data-cell="folder-sub"><?= stackman_folder_sub($row['count'], $row['running']) ?></span>
              </span>
            </div>
          </span>
          ...
          <span class="stackman-cell stackman-num" role="gridcell" data-stat="cpu"><span class="stackman-statv">—</span></span>
          <span class="stackman-cell stackman-num" role="gridcell" data-stat="mem"><span class="stackman-statv">—</span></span>
...
<?
      ...
      if ($expandable):
?>
      <div class="stackman-group stackman-group--stack" role="presentation" data-stack-group="<?= htmlspecialchars($s['name']) ?>">
<?
      endif;
?>
        <div class="stackman-row stackman-stack-row<?= $row['folder'] !== '' ? ' stackman-nested' : '' ?>"
          ...
          <span class="stackman-cell stackman-cell--name stackman-stack-name" role="gridcell">
            <div class="stackman-namebox">
              <? if ($expandable): ?>
                <button type="button" class="stackman-chevron"
              ...
              <? else: ?>
                <span class="stackman-chevron stackman-chevron--empty"></span>
              <? endif; ?>
              ...
              <span class="stackman-iconwrap">
                ...
                <span class="stackman-spinner"><i class="fa fa-refresh fa-spin"></i></span>
                <? if ($canRun && $s['parses']): ?>
                  <span class="stackman-dot<?= $s['running'] ? ' stackman-dot--up' : '' ?>"></span>
                <? endif; ?>
              </span>
          ...
          <span class="stackman-cell stackman-cell--services" role="gridcell">
            <? if (!$s['hasFile']): ?>
          ...
          <span class="stackman-cell stackman-cell--state" role="gridcell" data-cell="state"><?= stackman_state_pill($s, $canRun) ?></span>
          <span class="stackman-cell stackman-cell--address stackman-addrcell" role="gridcell" data-cell="address"><?=
            stackman_address_html(stackman_merged_addresses(stackman_stack_containers($s)))
          ?></span>
          ...
          <span class="stackman-cell stackman-num" role="gridcell" data-stat="cpu">
            <span class="stackman-statv">—</span>
            <span class="stackman-spark" data-spark="cpu"></span>
          </span>
          <span class="stackman-cell stackman-num" role="gridcell" data-stat="mem">
            <span class="stackman-statv">—</span>
            <span class="stackman-spark" data-spark="mem"></span>
          </span>
          <span class="stackman-cell stackman-num" role="gridcell" data-stat="net">
            <span class="stackman-statv stackman-net">—</span>
            <span class="stackman-spark" data-spark="net"></span>
          </span>
          <span class="stackman-cell stackman-num" role="gridcell" data-stat="gpu">
            <span class="stackman-statv">—</span>
            <span class="stackman-spark" data-spark="gpu"></span>
          </span>
        </div>
<?
      ...
      // .stackman-group--stack instead (tried earlier) hides the stack row's
...
?>
        ...
        <div class="stackman-row stackman-container-row"
          ...
          <span class="stackman-cell stackman-cell--name stackman-stack-name" role="gridcell">
            <div class="stackman-namebox stackman-namebox--child">
              <span class="stackman-iconwrap">
                <!-- A real button now, not a dead <span> — this is the menu
                     ...
                     heading. -->
                <button type="button" class="stackman-icon stackman-icon--child"
                        data-menu="container"
                        data-stack="<?= htmlspecialchars($s['name']) ?>"
                  ...
                  <?= stackman_icon_tile(
                        stackman_icon_resolve($kid['icon'], $s['dir'], $kid['image'],
                                              $kid['service'], $s['name']),
                        $kid['service'] !== '' ? $kid['service'] : $kid['name']) ?>
                </button>
                <span class="stackman-spinner"><i class="fa fa-refresh fa-spin"></i></span>
                ...
                <? if ($canRun): ?>
                  <span class="stackman-dot<?= $kid['state'] === 'running' ? ' stackman-dot--up' : '' ?>"></span>
                <? endif; ?>
              </span>

              <span class="stackman-nameinfo">
                <span class="stackman-name-text"><?= htmlspecialchars($kid['service']) ?></span>
                <? if ($kid['name'] !== '' && $kid['name'] !== $kid['service']): ?>
                  <span class="stackman-sub"><?= htmlspecialchars($kid['name']) ?></span>
                <? endif; ?>
              </span>
            </div>
          </span>
          ...
          <span class="stackman-cell stackman-cell--services stackman-image" role="gridcell">
            <? if ($kid['image'] !== ''): ?>
              <span title="<?= htmlspecialchars($kid['image']) ?>"><?= htmlspecialchars($kid['image']) ?></span>
            <? else: ?>
              <span class="stackman-sub"><?= _('not created yet') ?></span>
            <? endif; ?>
          </span>

          <span class="stackman-cell stackman-cell--state" role="gridcell" data-cell="state"><?= stackman_container_pill($kid) ?></span>
          <span class="stackman-cell stackman-cell--address stackman-addrcell" role="gridcell" data-cell="address"><?=
            stackman_address_html($kid['id'] !== ''
              ? (stackman_container_net()[$kid['id']]['addresses'] ?? [])
              : [])
          ?></span>
 ...
 */
function stackman_state_snapshot(): array {
  $canRun = stackman_can_run();
  $states = stackman_stack_states();
  ...
  foreach ($states as $name => $s) {
    ...
    $mine       = stackman_stack_containers($s);
    $containers = [];
    $seen       = [];
    foreach ($mine as $c) {
      $service = $c['service'] !== '' ? $c['service'] : $c['name'];
      ...
      $containers[$key] = [
        'container' => $c['name'],
        'state'     => $c['state'],
        'html'      => stackman_container_pill(['exists' => true] + $c),
        ...
        // render time.
        'address'   => stackman_address_html(stackman_container_net()[$c['id']]['addresses'] ?? []),
      ];
    }
  ...
  foreach (stackman_folder_names() as $folder) {
    ...
    $folders[$folder] = [
      'running' => $up,
      'html'    => stackman_folder_sub($count, $up),
    ];
  }
  ...
  return [
    ...
    'notCreated' => stackman_container_pill(['exists' => false]),
    // Likewise for the address cell of a container that no longer exists.
    'noAddress'  => stackman_address_html([]),
  ];
}
?>
