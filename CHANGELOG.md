...
- A new Merge button beside Add stack lets you fold two or more stacks into one — the way an application and its database usually arrive when converted from separate templates. A five-step window walks through picking the stacks, choosing which one keeps its name and history, deciding what to do about anything that clashes (a shared container name, a claimed port, storage Docker manages under the old name), and reviewing every address it can safely rewrite so the two talk to each other directly once merged. Nothing is written until the last step, which lists exactly what will happen; the stacks left behind are untouched and carry a "folded into" marker with a one-press Remove once you've checked the merged stack works. Needs a wider window — the tool is not offered below 990px.
