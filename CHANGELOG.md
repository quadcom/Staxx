...
- Every tooltip on the stack list, from the state chips and badges to the small buttons on a row, now opens as the same styled hover card the update chips use, instead of the browser's own plain tooltip. The card also appears when a control is reached with the keyboard.
